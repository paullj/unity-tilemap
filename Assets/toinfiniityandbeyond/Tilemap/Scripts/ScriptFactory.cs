	...
	private static void CreatScriptableToolScript ()
	{
		...
		if (File.Exists (path) == false)
		{
			using (StreamWriter outfile = new StreamWriter (path))
			{
				...
				outfile.WriteLine ("		public override bool Use (Point point, ScriptableTile tile, TileMap map)");
				outfile.WriteLine ("		{");
	...
	private static void CreateTileScript ()
	{
		...
		if (File.Exists (path) == false)
		{
			using (StreamWriter outfile = new StreamWriter (path))
			{
				...
				outfile.WriteLine ("		}");
				outfile.WriteLine ("		public override Texture2D GetTexture (TileMap tilemap, Point position = default (Point))");
				outfile.WriteLine ("		{");
				outfile.WriteLine ("			if (texture == null)");
				outfile.WriteLine ("				RebuildTexture ();");
				outfile.WriteLine ("			return texture;");
				outfile.WriteLine ("		}");
				outfile.WriteLine ("		public override Color [] GetColors (TileMap tilemap, Point position = default (Point))");
				outfile.WriteLine ("		{");
